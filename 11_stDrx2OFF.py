#####################################################
# 	Script for: Measure timing - Drx to OFF  state	#		
# 	Output: written to XL file						#
# 	Input files: VUC Drx2stOFF.txt					#
#####################################################
...
def pharseIt():
    ...
    pharseFile = open('VUC Drx2stOFF.txt', 'r')
    print "File Open for reading"
    lines = pharseFile.readlines()
    for line in lines:
    
        if "Cur State: StOFF, Prev State: StDrx" in line:
            start_point(line)
...
def copy2XL():
    ...
    workbook = xlsxwriter.Workbook('stDrx2OFF_multi_Timing.xlsx')
