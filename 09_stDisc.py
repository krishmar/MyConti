#####################################################
# 	Script for: Measure timing for stDisc state		#		
# 	Output: written to XL file						#
# 	Input files: VUC stDisc.txt 					#
#####################################################
...
def pharseIt():
    ...
    pharseFile = open('VUC stDisc.txt', 'r')
    print "File Open for reading"
    lines = pharseFile.readlines()
    for line in lines:
    
        if "Cur State: StDrx, Prev State: StDisc" in line:
            start_point(line)
...
def copy2XL():
    ...
    workbook = xlsxwriter.Workbook('stDisc_multi_Timing.xlsx')
