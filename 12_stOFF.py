#####################################################
# 	Script for: Measure timing for stOFF state		#		
# 	Output: written to XL file						#
# 	Input files: VUC stOFF.txt						#
#####################################################
...
def pharseIt():
    ...
    pharseFile = open('VUC stOFF.txt', 'r')
    print "File Open for reading"
    lines = pharseFile.readlines()
    for line in lines:
    
        if "Cur State: StOFF, Prev State: VehOffEntry" in line:
            start_point(line)
...
def copy2XL():
    ...
    workbook = xlsxwriter.Workbook('stOFF_multi_Timing.xlsx')
