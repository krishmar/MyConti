#####################################################
# 	Script for: Measure timing for stOFFData state	#		
# 	Output: written to XL file						#
# 	Input files: VUC stOFFData.txt					#
#####################################################
...
def pharseIt():
    ...
    pharseFile = open('VUC stOFFData.txt', 'r')
    print "File Open for reading"
    lines = pharseFile.readlines()
    for line in lines:
    
        if "Cur State: StDrx, Prev State: StOffData" in line:
            start_point(line)
...
def copy2XL():
    ...
    workbook = xlsxwriter.Workbook('stOFFData_multi_Timing.xlsx')
