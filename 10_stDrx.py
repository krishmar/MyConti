#####################################################
# 	Script for: Measure timing for stDrx state		#		
# 	Output: written to XL file						#
# 	Input files: VUC Drx.txt						#
#####################################################
...
def pharseIt():
    ...
    pharseFile = open('VUC Drx.txt', 'r')
    print "File Open for reading"
    lines = pharseFile.readlines()
    for line in lines:
    
        if "Cur State: StDrx, Prev State: VehOffEntry" in line:
            start_point(line)
...
def copy2XL():
    ...
    workbook = xlsxwriter.Workbook('stDrx_multi_Timing.xlsx')
